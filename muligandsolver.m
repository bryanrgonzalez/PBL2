...
k = 1;
for i = 1.5:0.1:2.5
[tout, yout] = ode15s(@(t,x) DE(t,x,[kq, kminus, kp, kplus, 1, thetamu, kaplus, i, kaminus, kbplus, kbminus,kr,kf,thetazeta,zetaminus,zetaplus,thetav,vminus,vplus,thetamuv,thetaa,thetazetav,thetazetamu, kgplus, kgminus,A, kcon,kint]), tspan, yinit);
...
title('Parameter Sweep of \mu', 'Total Concentration of Activated Receptor vs. Time with Internalization');
k = k+1;
end;
...
title('Parameter Sweep of \mu', 'Concentration of α† vs. Time with Internalization');
yolo = yout(:,11);
...
figure(3)
plot(muarray,maxI,'-x')
title('Peak α† concentration vs. μ with internalization')

figure(3)
plot(muarray,maxI,'k-o')
title('\mu Parameter vs. Peak [α†]', 'with Internalization')
xlabel('\mu')
