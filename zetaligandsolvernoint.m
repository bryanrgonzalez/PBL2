...
k = 1;
for i = 0.5:0.1:1.5
[tout, yout] = ode15s(@(t,x) DE(t,x,[kq, kminus, kp, kplus, muminus, thetamu, kaplus, muplus, kaminus, kbplus, kbminus,kr,kf,thetazeta,1e-3,i,thetav,vminus,vplus,thetamuv,thetaa,thetazetav,thetazetamu, kgplus, kgminus,A]), tspan, yinit);
...
xlim([1e-3 1e4])
ylim([0 0.4])
Legend{k}=strcat('\zeta=', num2str(i*1000));
title('Parameter Sweep of \zeta', 'Total Concentration of Activated Receptor vs. Time without Internalization');
k = k+1;
end
...
xlim([1e-3 1e4])
ylim([0 2.3])
Legend2{r}=strcat('\zeta=', num2str(i*1000));
title('Parameter Sweep of \zeta', 'Concentration of α† vs. Time without Internalization');
yolo = yout(:,11);
...
ylim([1.75 2.05])
title('\zeta Parameter vs. Peak [α†]', 'without Internalization')
