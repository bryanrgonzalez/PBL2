...
k = 1;
for i = 0.5:0.5:5
[tout, yout] = ode15s(@(t,x) DE(t,x,[kq, kminus, kp, kplus, muminus, thetamu, kaplus, muplus, kaminus, kbplus, kbminus,kr,kf,thetazeta,zetaminus,zetaplus,thetav,1,i,thetamuv,thetaa,thetazetav,thetazetamu, kgplus, kgminus,A]), tspan, yinit);
...
ylim([0 0.4])
Legend{k}=strcat('\nu=', num2str(i));
title('Parameter Sweep of \nu', 'Total Concentration of Activated Receptor vs. Time without Internalization');
k = k+1;
end;
...
ylim([0 2.3])
Legend2{r}=strcat('\nu=', num2str(i));
title('Parameter Sweep of \nu', 'Concentration of α† vs. Time without Internalization');
yolo = yout(:,11);
...
ylim([1.7 2.2])
title('\nu Parameter vs. Peak [α†]', 'without Internalization')
