...
xlim([1e-3 1e4])
ylim([0 0.35])
Legend{k}=strcat('μ= ', num2str(i*1000));
title('Parameter Sweep of μ -', 'Total Concentration of Activated Receptor vs. Time with Internalization');
k = k+1;
end;
...
ylabel('Concentration (1e-10 M)')
xlim([1e-3 1e4])
Legend2{r}=strcat('μ= ', num2str(i*1000));
title('Parameter Sweep of μ', 'Concentration of α† vs. Time with Internalization');
r = r+1;
end;
