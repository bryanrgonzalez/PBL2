...
k = 1;
for i = 0.5:0.5:5
[tout, yout] = ode15s(@(t,x) DE(t,x,[kq, kminus, kp, kplus, muminus, thetamu, kaplus, muplus, kaminus, kbplus, kbminus,kr,kf,thetazeta,zetaminus,zetaplus,thetav,1,i,thetamuv,thetaa,thetazetav,thetazetamu, kgplus, kgminus,A,kcon, kint]), tspan, yinit);
semilogx(tout,yout(:,7)+yout(:,9),'color',rand(1,3))
hold on
xlabel('Time (s)')
ylabel('Concentration (1e-10 M)')
xlim([1e-3 1e4])
Legend{k}=strcat('ν= ', num2str(i*1000));
title('Parameter Sweep of ν -', 'Total Concentration of Activated Receptor vs. Time with Internalization');
k = k+1;
end;
...
ylabel('Concentration (1e-10 M)')
xlim([1e-3 1e4])
Legend2{r}=strcat('ν= ', num2str(i*1000));
title('Parameter Sweep of ν', 'Concentration of α† vs. Time with Internalization');
r = r+1;
end;
